...
- **全站夜间模式**：CSS 滤镜反色，图片 / 视频 / 画布自动还原原色（只比白天略暗一档，不受色温 / 灰度影响）
- **自动切换**：两种时间表
  - 跟随日出日落：填入经纬度（纯本地天文算法，不需要联网），支持自定义偏移；极昼极夜自动降级为固定时段
  - 固定时段：例如 19:00 – 07:00
- **定位辅助**：点「获取我的位置」自动填经纬度，并回显最近城市（内置 3300 城精简库，本地匹配，不联网反查）
- **手动切换**：popup 一键切换 自动 / 黑夜 / 白天。选「黑夜」或「白天」只是**临时覆盖**，到下一个日出/日落（或固定时段边界）会自动回到「自动」
- **站点三态**：popup 底部对当前网址可选 跟随自动 / 强制夜间 / 强制白天，分别对应「不落名单 / 始终夜间 / 始终白天」，随时可切回跟随自动
- **工具栏徽标**：黑夜时显示 ON，白天时显示 OFF
- **总开关最顶层**：设置页关闭「启用」后，popup 的昼夜模式与站点三态、快捷键、右键菜单全部暂不生效——相关按钮会置灰并给出提示，重新开启后恢复
...
- **快捷键**：`Alt+Shift+D` 切换昼夜；站点加入 / 移出始终白天名单可在 `chrome://extensions/shortcuts` 自行绑定
...
- 地图、在线设计工具、PDF 查看器等依赖精确颜色的页面效果差，请加入始终夜间名单
- 页面使用 `position: fixed` 的吸顶导航在部分站点可能表现异常，可在「高级」中切换滤镜应用范围，或加入始终夜间名单
- 「保留图片与视频的原始色彩」开启时，媒体被精确还原（只比白天略暗 8%），不受色温 / 灰度 / 亮度 / 对比度影响；关闭则图片跟随整页一起变暗。极鲜艳色可能因 CSS 滤镜的色域裁剪有难以察觉的微小差异。
